...
st.markdown("""
    ...
    .station-info { margin-left: 45px; background: rgba(255,255,255,0.05); padding: 10px; border-radius: 8px; border-left: 3px solid #30363d; }
    .station-name { font-weight: bold; font-size: 20px; color: #f0f6fc; }
    .station-details { font-size: 14px; color: #8b949e; margin-top: 4px; }
    .live-tag { color: #ff7b72; font-weight: bold; font-size: 14px; animation: blinker 1.2s linear infinite; }
...
# 3. STATION DATA (Arrival, Departure, PF, Distance)
# Rationale: Using realistic Tamil Nadu railway section data
railway_data = [
...
# 4. MAIN DASHBOARD
st.title("🚅 AI-Powered Precise Toy Train Simulator")

if st.button("🛰️ Start Live Tracking Simulation"):
    tracker_placeholder = st.empty()
    
    # Simulation Loop
    for idx in range(len(railway_data)):
        with tracker_placeholder.container():
            col_track, col_ai = st.columns([1.2, 1])
            
            with col_track:
                st.subheader(f"📍 GPS Live Tracking: {train_id}")
                
                # Toy Train Vertical Movement Logic
                track_html = f"""
                <div class="track-container">
                    <div class="vertical-line"></div>
                    <div class="active-line" style="height: {idx * 135}px;"></div>
                    <div class="train-toy" style="top: {idx * 135}px;">🚅</div>
                """
                for i, station in enumerate(railway_data):
                    is_active = (i <= idx)
                    dot_style = "active-dot" if is_active else ""
                    live_label = "<span class='live-tag'>● LIVE</span>" if i == idx else ""
                ...
                track_html += "</div>"
                components.html(track_html, height=750)

            with col_ai:
                st.subheader("📢 AI Precision Commands")
                
                # Dynamic Analytics based on current location
                ...
                st.markdown(f"""<div class="status-box health-panel">
                    ...
                    DQN Analysis: Predictive maintenance status OK.
                </div>""", unsafe_allow_html=True)
                
                st.markdown(f"""<div class="status-box energy-panel">
                    <b>⚡ Energy Optimization: Active</b><br>
                    Efficiency: <b>{random.randint(12, 22)}% Power Saved</b> via Precise Throttle.
                </div>""", unsafe_allow_html=True)
            
            # Simulated Time Delay between stations
            time.sleep(3) 
            
    st.balloons()
    st.success("Simulation Complete: Maximum Throughput Achieved.")import streamlit as st
import time
import streamlit.components.v1 as components
import random
