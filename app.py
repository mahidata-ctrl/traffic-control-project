...
# High-Visibility Dark Theme with Vertical Animation CSS
st.markdown("""
    <style>
    ...
    /* The Blue Progress Line (This animates!) */
    .active-line { 
        position: absolute; 
        left: 75px; 
        top: 0; 
    ...
    /* Toy Train Icon Movement (Animation) */
    .train-toy { 
        font-size: 40px; 
        position: absolute; 
        left: 0px; 
...
if st.button("🛰️ Sync Live Satellite & Start AI Animation"):
    tracker_placeholder = st.empty()
    
    # Simulation Loop: Moving the toy train station by station
    for idx in range(len(railway_data)):
        with tracker_placeholder.container():
            ...
            with col_track:
                ...
                # Toy Train Vertical Movement Logic with Animation
                track_html = f"""
            ...
            with col_ai:
                ...
                # Dynamic Enhancement Logic based on movement
            ...
            # Simulation delay to allow CSS transitions to complete
            time.sleep(3) 
