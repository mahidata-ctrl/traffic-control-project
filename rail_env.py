...
class TrainTrafficEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Reward Logic: The most important part for Research Paper!
        # Reward = Moving fast + maintaining safe distance
        reward = (speed * 0.5) 
        if dist < 20: reward -= 50  # Safety penalty (Collision avoidance)
        if dist > 20 and dist < 40: reward += 20 # Optimal headway (Throughput)

        done = pos >= 1000 # Section length 1km
